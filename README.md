...
2. У неё на «Сегодня» появляется крупный пункт **«Записать сессию»**: код или имя клиента,
   дата и время начала, по желанию — окончание (в том числе на другой день).
3. **«Сохранить»** → сразу экран с одной крупной кнопкой **«Отправить Иоанн»** — и в мессенджер.
4. У вас ссылка открывается отдельным заметным экраном: клиент (в закрытой области — скрыт,
   пока не коснёшься), начало, конец, область. Кнопка **«Добавить в план»** ставит сессию в план
   сразу, минуя «Входящие»:
   - если время накладывается на постоянный блок или другое дело, приложение спокойно скажет,
     ...
     **«Изменить время»**;
   - знакомый код клиента — сессия привязывается к нему, растёт его счётчик, запускается его
     цепочка (подготовка, запись, разбор); новый код — приложение предложит создать клиента
     в закрытой области.
...
- в календарь телефона (`.ics`) уходит только «Встреча»;
