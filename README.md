...
3. Если ни то, ни другое не сработало — **скачивает файл**: «Файл скачан в Загрузки. Откройте
   Загрузки → нажмите на файл → выберите Яндекс.Календарь».

**Если календарь не открывается — «Диагностика календаря»** (Меню → Настройки). Экран показывает:
умеет ли браузер `navigator.share` (true/false), что ответил `navigator.canShare` для файла `.ics`
(true/false/недоступно), каким способом прошла последняя попытка, полный текст последней ошибки,
проверку самого файла (переводы строк CRLF, блоки VCALENDAR/VEVENT, UID, PRODID, DTSTAMP, DTSTART,
DTEND) и тип файла, а ниже — историю последних 5 попыток с датой и временем (хранится на телефоне).

Как найти рабочий способ на своём телефоне:
1. Нажмите **«Пробная попытка»** — уйдёт событие «Проверка планировщика (можно удалить)» на завтра, 10:00.
2. Вернитесь в приложение и отметьте, **что произошло на экране** (открылся календарь / началась
   загрузка / ничего / что-то другое).
3. Если не сработало — попробуйте по очереди кнопки конкретных способов: **share с файлом**,
   **ссылка `<a href="blob:…" target="_blank">`**, **прямой переход по blob-ссылке**, **скачивание**.
4. Когда календарь открылся — нажмите **«Использовать этот способ»**. Он запомнится в
   **Настройки → «Как передавать в календарь»** (там же его можно сменить вручную).
5. Если не помог ни один способ — нажмите **«Скопировать текст»** и пришлите его разработчику:
   по нему видно, на каком шаге и с какой ошибкой всё останавливается.
