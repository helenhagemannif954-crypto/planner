...
семейную, рабочую или клиентскую, — приложение сразу открывает системный выбор приложения
для файла события. Выберите **Яндекс.Календарь** — и событие добавится в общий календарь,
который видит и жена. Выбор приложения — последний шаг, убрать его нельзя: это правило
браузера, а не недоработка. Если браузер не умеет передать файл напрямую, файл скачается —
нажмите в уведомлении **«Открыть»**. Так же ведут себя запуск цепочки (в календарь уходит сама
встреча, например сессия) и «Добавить в план» у записи сессии от жены.

**Переключатель «Автоматически предлагать календарь»** (Меню → Настройки, включён сразу).
Включён — выбор приложения появляется сам сразу после сохранения задачи со временем.
Выключен — ничего не всплывает; отправить задачу в календарь можно вручную: карточка задачи →
**⋯ → «В календарь»**. Задачи без времени в календарь сами не предлагаются.
