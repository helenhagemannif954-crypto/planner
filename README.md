...
семейную, рабочую или клиентскую, — приложение сразу сохраняет файл события `.ics` обычной
загрузкой. **Откройте его из уведомления о загрузке (или из папки «Загрузки») и выберите
Яндекс.Календарь** — событие добавится в общий календарь, который видит и жена. Выбор
приложения при открытии — последний шаг, убрать его нельзя: так устроен Android.
Системное «Поделиться» для этого не используется: оно показывает мессенджеры, а не календари.
Так же ведут себя запуск цепочки (в календарь уходит сама встреча, например сессия)
и «Записать сессию» у жены.

В карточке любой задачи с датой (со временем или на весь день) и у задачи со сроком всегда
видна кнопка **«Добавить в календарь»**; после нажатия под ней появляется подсказка, где файл.
